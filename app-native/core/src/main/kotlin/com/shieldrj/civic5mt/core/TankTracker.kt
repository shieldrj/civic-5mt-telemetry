...
object TankRules {
     ...
     *
     * About two thirds of a gallon. No refuel is smaller, and no amount of fuel sloshing in
     * a tank is bigger - especially after smoothing.
     */
    const val FILL_RISE_PERCENT = 5.0
    ...
    /** Below this, distance over fuel is not yet an economy figure. */
    const val MIN_GALLONS_FOR_MPG = 0.5
...
/**
     ...
     *
     * The point of using an average is that it holds still. A lifetime record with four miles
     * in it does not: every mile driven moves it by a quarter. Making range fall back to a
     * figure that swings defeats the whole purpose of not using the 30-second one.
     */
    const val MIN_LIFETIME_MILES_FOR_RANGE = 50.0
    ...
    fun mpgForRange(tankMpg: Double?, lifetimeMpg: Double, lifetimeMiles: Double): Double = when {
        tankMpg != null && tankMpg > 5 -> tankMpg
        lifetimeMpg > 5 && lifetimeMiles >= MIN_LIFETIME_MILES_FOR_RANGE -> lifetimeMpg
